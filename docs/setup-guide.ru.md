...
> **Важно: `dbrain restart` ≠ пересоздание мозга.** Перезапуск перезапускает только Python-процесс бота; долгоживущая сессия Claude Code в tmux при этом **сознательно выживает** (`KillMode=process`) — именно она держит вас на подписке, а не на платных токенах. Поэтому настройки, которые читаются только при СОЗДАНИИ сессии — `CLAUDE_MODEL`, `mcp-config.json`, системный промпт — после простого `dbrain restart` НЕ применятся. Чтобы они вступили в силу, пересоздайте мозг — **обе сессии**: основную и cron (напоминания крутятся во второй сессии `${brain}_cron` с теми же настройками):
> ```bash
> B="$(cat ~/.dbrain/brain.name)"
> tmux kill-session -t "$B"            # основной мозг
> tmux kill-session -t "${B}_cron" 2>/dev/null   # cron-мозг (может ещё не существовать)
> # бот поднимет новые сессии при следующем сообщении / cron-запуске
> ```
> (Изменения в `.env` для самого бота — токены, `ALLOWED_USER_IDS`, `TZ`, `CRON_ENABLED` — наоборот, применяются обычным `dbrain restart`.)
...
Как подключить: положите файл `mcp-config.json` в корень проекта (`~/projects/agent-second-brain/mcp-config.json`) — он передаётся в Claude Code через `--mcp-config` при СОЗДАНИИ сессии. Поэтому простого `dbrain restart` недостаточно (мозг переживает рестарт): после добавления **пересоздайте мозг** — см. врезку про `tmux kill-session` выше.
