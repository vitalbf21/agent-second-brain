...
> **Почему это важно (июнь 2026).** С 15 июня 2026 Anthropic переводит `claude -p` / headless-запуски на отдельный платный кредит Agent SDK с поштучной оплатой. Agent Second Brain **v3.1** обходит это полностью: он гоняет постоянную *интерактивную* сессию, которая остаётся на твоей фиксированной подписке Pro/Max. Никакой поштучной оплаты и неожиданных счетов - цена выше остаётся ценой. (Если всё же захочешь API-путь - встроен опциональный escape-hatch `DBRAIN_MODE=router`.)
